...
export const recommendProgram = async (
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to get program recommendation" });
  }
};
  ...
  try {
    ...
    if (score === undefined || !breakdown_percentiles || !raw_values) {
      res
        ...
        .json({
          success: false,
          error: "Complete performance data is required.",
        });
      return;
    }
    ...
    res
      ...
      .json({
        success: false,
        error: "Failed to generate coach advice from AI microservice.",
      });
  }
};
...
export const getSessions = async (
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch chat sessions." });
  }
};
...
export const getSessionMessages = async (
  ...
  try {
    ...
    if (session.user_id !== userId) {
      res
        .status(403)
        .json({ success: false, error: "Unauthorized to view this session." });
      return;
    }

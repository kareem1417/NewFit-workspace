...
const getPercentileForTest = async (testId, rawValue, higherIsBetter, userLevel, userWeight, userAgeGroupId) => {
 ...
 */
const getUserCompositeScore = async (userId, testIds, userLevel, userWeight, userAgeGroupId) => {
    ...
    for (const testVal of latestSnapshot.snapshot_test_values) {
        const percentile = await getPercentileForTest(testVal.attribute_test_id, Number(testVal.value), testVal.attribute_tests?.higher_is_better ?? true, userLevel, userWeight, userAgeGroupId);
...
const getLeaderboard = async (req, res) => {
    try {
        ...
        const weightClass = req.query.weight_class || currentUserProfile.weight_class;
        ...
        if (type === 'most_improved') {
            ...
            leaderboardData = scores.filter(s => s !== null);
            leaderboardData.sort((a, b) => b.score - a.score);
            leaderboardData = leaderboardData.map((item, idx) => ({ ...item, rank: idx + 1 }));
        }
...
// Helper function
async function getCompositeScoreFromSnapshot(snapshotId, testIds, level, weightClass, ageGroupId) {
    ...
    if (!snapshot || snapshot.snapshot_test_values.length === 0)
        return 0;
    let totalPercentile = 0;
    for (const tv of snapshot.snapshot_test_values) {
        const pct = await getPercentileForTest(tv.attribute_test_id, Number(tv.value), tv.attribute_tests?.higher_is_better ?? true, level, weightClass, ageGroupId);
        totalPercentile += pct;
    }

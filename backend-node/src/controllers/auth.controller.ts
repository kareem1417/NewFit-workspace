  ...
  try {
    ...
    if (!passwordRegex.test(password)) {
      res.status(400).json({
        success: false,
        error:
          "Weak password. Must be at least 8 characters long, containing 1 uppercase letter, 1 lowercase letter, and 1 number.",
      });
      return;
    }
    ...
    if (!date_of_birth) {
      res
        .status(400)
        .json({ success: false, error: "Validation error — DOB required." });
      return;
    }
...
export const login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!email || email.trim() === "") {
      res
        .status(400)
        .json({ success: false, error: "Validation error — Email required." });
      return;
    }

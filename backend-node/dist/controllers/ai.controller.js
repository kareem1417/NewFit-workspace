...
const askQuestion = async (req, res, next) => {
    try {
        ...
        if (currentSessionId) {
            ...
            const existingSession = await prisma_1.prisma.chat_sessions.findUnique({
                where: { id: currentSessionId },
            });
            if (!existingSession) {
                return next(new AppError_1.AppError("Session not found", 404));
            }
            if (existingSession.user_id !== userId) {
                return next(new AppError_1.AppError("Forbidden — Session belongs to another user", 403));
            }
        ...
        next(new AppError_1.AppError("Failed to get AI response", 500));
    }
};
...
const recommendProgram = async (req, res, next) => {
    try {
        const userId = req.user?.sub;
        ...
        if (!user.user_metrics) {
            return next(new AppError_1.AppError("User metrics not found. Please complete onboarding first.", 400));
        }
        const primaryProfile = user.user_sport_profiles[0];
        let metrics = user.user_metrics;
        // 🎯 2. السحر هنا: لو اليوزر بعت تعديلات، نحدث الداتا بيز الأول قبل ما نكلم الموديل
        if (overrides && Object.keys(overrides).length > 0) {
            metrics = await prisma_1.prisma.user_metrics.update({
        ...
        console.error("ML Recommend Error:", error);
        next(new AppError_1.AppError("Failed to get program recommendation", 500));
    }
};
...
const getCoachAdvice = async (req, res, next) => {
    try {
        ...
        if (score === undefined || !breakdown_percentiles || !raw_values) {
            return next(new AppError_1.AppError("Complete performance data is required.", 400));
        }
        ...
        next(new AppError_1.AppError("Failed to generate coach advice from AI microservice.", 500));
    }
};
...
const getSessions = async (req, res, next) => {
    try {
        ...
        next(new AppError_1.AppError("Failed to fetch chat sessions.", 500));
    }
};
...
const getSessionMessages = async (req, res, next) => {
    try {
        ...
        if (session.user_id !== userId) {
            return next(new AppError_1.AppError("Unauthorized to view this session.", 403));
        }
        ...
        const formattedMessages = await Promise.all(messages.map(async (msg) => {
            ...
            if (Array.isArray(msg.suggested_program_ids) &&
        ...
        next(new AppError_1.AppError("Failed to fetch session messages.", 500));
    }
};
